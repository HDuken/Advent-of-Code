...
void solve()
{
    ...
    cout << "Result = " << get_result(elves) << endl;
}
...
void move(vector<Elf> &elves, Grid &grid)
{
    ...
    // Check if a pair of elves want to move to the same position
    // If yes, both of them won't move
    // for (size_t i = 0; i < elves.size() - 1; i++) {
    //     for (size_t j = i + 1; j < elves.size(); j++) {
    //         if (elves[i].next_p_ == elves[j].next_p_) {
    //             elves[i].next_p_ = elves[i].p_;
    //             elves[j].next_p_ = elves[j].p_;
    //         }
    //     }
    // }
    // for (auto &elf : elves) {
    //     if (elf.p_ == elf.next_p_)
    //         continue;
    //     grid.set(elf.next_p_);
    //     grid.unset(elf.p_);
    //     elf.p_ = elf.next_p_;
    // }
}
