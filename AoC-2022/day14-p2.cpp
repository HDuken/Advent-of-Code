...
const int FALLING = 0, STOP = 1, OFFMAP = 2, FULL = 3;

Paths read_input();

void solve()
{
    Paths paths = read_input();

    // Get the range of the grid map
    ...
    ymax += 2;
    xmax = (xmax < SOURCE + ymax) ? SOURCE + ymax : xmax;
    xmin = (xmin > SOURCE - ymax) ? SOURCE - ymax : xmin;

    Grid grid(ymax + 1, vector<char>(xmax - xmin + 3, AIR));
    ...
    // Run simulation
    while (true) {
        auto result = sim_falling_sand();
        if (result == OFFMAP || result == FULL)
            break;
    }
